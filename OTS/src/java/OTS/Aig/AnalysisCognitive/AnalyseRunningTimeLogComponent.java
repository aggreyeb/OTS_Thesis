 ...
 * @author Eb
 */
public class AnalyseRunningTimeLogComponent implements OTS.Aig.ITestItemGenerationComponent  {
    ...
    // int[] intitialDataSets= new int[]{1000};
     private final double InitialTimeSpent=1;
     private  double intitialDataSet=1000;
     private final double requiredDataSet=1000000;
    
     
     
    public AnalyseRunningTimeLogComponent(DataSource mySqlDataSource) {
    ...
    public String ConstructStimulus() {
        String stimulus="";
       String template="An algorithm a  %s implemented for one of the methods of %s"
               + " spent  %s ms to process %s data items.";
    ...
    public String PrepareStem() {
        String stemTemplate="How much time will be spent to process %s. Assuming"
                + " computer processing T(n) of the algorithem as as function"
                + " of time complextiy O(f(n) is  T(n)=cnlog(n) where "
                + " c is constant and n the number of data items";
   ...
   protected  Map CalculateRunningTime(){
       Map map= new HashMap();
        ...
        //Subsititute 2 in 1
         ...
         double requiredTimeSpent=InitialTimeSpent/(intitialDataSet * Math.log10(intitialDataSet)) *requiredDataSet * Math.log10(requiredDataSet);
